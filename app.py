...
st.title("КГУ Общеобразовательная школа №19")
st.title("📊 AI-Анализатор СОР/СОЧ и Нулевого Среза")
...
if uploaded:
    ...
    # фильтруем строки СОР/СОЧ
    mask = df_raw[0].astype(str).str.contains("СОР|СОЧ", case=False, na=False)
    df = df_raw[mask].copy()
    df = df.reset_index(drop=True)

    # УДАЛЯЕМ строки, содержащие заголовки типа "Анализ результатов..."
    df = df[~df[0].astype(str).str.contains("Анализ результатов", case=False, na=False)]
    mask = df_raw[0].astype(str).str.contains("СОР|СОЧ", case=False, na=False)
    df = df_raw[mask].copy()
    df = df.reset_index(drop=True)
    ...
    # Удаляем строки, где в названии работы есть служебный текст
    df = df[~df['Работа'].astype(str).str.contains("Анализ результатов", case=False, na=False)]
